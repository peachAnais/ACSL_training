...
int base_10_to_base_b(int base, int num){
...
int base_b_to_base_10(int base, int num){
    int res = 0;
    vector<int> digits = num_to_vector(num);
    int exponent = 0;
    
    for(int i = digits.size() - 1; i >= 0; i++){
        res = res + digits.at(i) * pow(base, exponent);
        exponent++;
    }
    return res;
}
...
int countLargestDigit(int num, int base, int start) {
    ...
    int start_in_10 = base_b_to_base_10(base, start);
    ...
    for(int i : num_in_base_10){
        int val = base_10_to_base_b(base, i);
